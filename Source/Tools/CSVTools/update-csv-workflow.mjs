...
async function main() {
  console.clear();
  console.log('='.repeat(60));
  console.log('CSV PROJECT MAP - CORE WORKFLOW');
  console.log('='.repeat(60));
  ...
  try {
    ...
    const orderScriptPath = path.join(
      __dirname,
    ...
    const sideEffectsScriptPath = path.join(
      __dirname,
    ...
    const executionContextScriptPath = path.join(
      __dirname,
      'updateExecutionContext.mjs'
    );
    await executeScript(executionContextScriptPath, 'Execution Context Extraction', {
      env: {
        CSV_PROJECT_MAP_PATH: snapshotPath,
      },
    });

    const errorHandlingScriptPath = path.join(
      __dirname,
      'updateErrorHandlingCoverage.mjs'
    );
    await executeScript(
      errorHandlingScriptPath,
      'Error Handling Coverage Extraction',
      {
        env: {
          CSV_PROJECT_MAP_PATH: snapshotPath,
        },
